...
bool CMoteur::Creation(HINSTANCE hInstance,int Largeur, int Hauteur, bool PleinEcran)
{
	...
	{
		if(IDNO == MessageBox(m_hWnd,"Le Moteur Audio a rencontré des problèmes lors de son initialisation, Voulez-vous Continuez sans prendre en compte le Moteur Audio ?","Echec Moteur Audio",MB_YESNO | MB_ICONQUESTION))
			return false;
	}
