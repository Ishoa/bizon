...
CVegetation::~CVegetation()
{
	...
	SAFE_DELETE_ARRAY(m_pArbre);
	SAFE_DELETE_ARRAY(m_pPlante);
	SAFE_DELETE_ARRAY(m_pFleur);
}
...
void CVegetation::RenduVegetation()
{
	...
	if(m_pTxArbre1.getDXTexture() != NULL)
	{
		// On active la texture d'un arbre
		m_pD3DDevice->SetTexture(0, m_pTxArbre1.getDXTexture());
	...
	if(m_pTxArbre2.getDXTexture() != NULL)
	{
		m_pD3DDevice->SetTexture(0, m_pTxArbre2.getDXTexture());
	...
	if(m_pTxArbre3.getDXTexture() != NULL)
	{
		m_pD3DDevice->SetTexture(0, m_pTxArbre3.getDXTexture());
	...
	if(m_pTxPlante.getDXTexture() != NULL)
	{
		m_pD3DDevice->SetTexture(0, m_pTxPlante.getDXTexture());
		for(int i=0; i<m_NbPlante ; i++)
		{
	...
	if(m_pTxFleur.getDXTexture() != NULL)
	{
		m_pD3DDevice->SetTexture(0, m_pTxFleur.getDXTexture());
		for(int i=0; i<m_NbFleur; i++)
		{
...
bool CVegetation::ChargerVegetation()
{
	...
	if(m_pFichierFleur != NULL)
		if(!m_pTxFleur.chargerTexture(m_pD3DDevice, m_pFichierFleur))
			return false;

	if(m_pFichierPlante != NULL)
		if(!m_pTxPlante.chargerTexture(m_pD3DDevice, m_pFichierPlante))
			return false;
		
	if(m_pFichierArbreType1 != NULL)
		if(!m_pTxArbre1.chargerTexture(m_pD3DDevice, m_pFichierArbreType1))
			return false;

	if(m_pFichierArbreType2 != NULL)
		if(!m_pTxArbre2.chargerTexture(m_pD3DDevice, m_pFichierArbreType2))
			return false;

	if(m_pFichierArbreType3 != NULL)
		if(!m_pTxArbre3.chargerTexture(m_pD3DDevice, m_pFichierArbreType3))
			return false;
