...
bool CMoteur::CreationSceneMoteur(CScene * pScene, int Largeur, int Hauteur, bool PleinEcran)
{
	...
	Console << "Attribution CScene" << endl;
	m_pScene = pScene;

	// Création de la scène à afficher
	m_pScene->SetRatioFenetre((float)m_LargeurFenetre/(float)m_HauteurFenetre);
	
	if(!m_pScene->Creation()) 
		return false;
...
// création du Moteur
bool CMoteur::Creation(HINSTANCE hInstance)
{
	...
	if(!(m_pMoteur3D && m_pPeripherique)) 
		return false;
...
bool CMoteur::GetDeviceLost()
{
	...
	{
		m_pScene->CreationObjet();
		//RestoreAllStates() : restauration de tous les états
	}
