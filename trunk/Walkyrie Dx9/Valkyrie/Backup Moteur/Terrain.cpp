...
CTerrain::CTerrain(CScene* pScene)
{
	...
	m_pTxHerbe = NULL;
	m_pTxTerre = NULL;
	m_pTxDensiteTerre = NULL;
	m_pTxEau = NULL;
	m_pTxDensiteEau = NULL;
...
// Création du terrain
bool CTerrain::CreationTerrain(char * FichierMapTerrain)
{
	if(!ChargerMapHauteur(FichierMapTerrain))
		return false;
	LisserMapHauteur();
...
{
	HRESULT hr;
	SAFE_RELEASE(m_pTxHerbe);
	hr = D3DXCreateTextureFromFile(m_pD3DDevice, FichierTextureSurface,        &m_pTxHerbe);
	if(FAILED(hr))
		return false;
	return true;
}
...
{
	HRESULT hr;
	SAFE_RELEASE(m_pTxTerre);
	hr = D3DXCreateTextureFromFile(m_pD3DDevice, FichierTextureTerre,        &m_pTxTerre);
	if(FAILED(hr))
		return false;
	SAFE_RELEASE(m_pTxDensiteTerre);
	hr = D3DXCreateTextureFromFile(m_pD3DDevice, FichierDensiteTerre, &m_pTxDensiteTerre);
	if(FAILED(hr))
		return false;
	return true;
}
...
bool CTerrain::SetEaux(char* FichierDensiteEau,char * FichierTextureEau, float fHauteurEau)
{
	...
	HRESULT hr;
	SAFE_RELEASE(m_pTxEau);
	hr = D3DXCreateTextureFromFile(m_pD3DDevice, FichierTextureEau,          &m_pTxEau);
	if(FAILED(hr))
		return false;
	SAFE_RELEASE(m_pTxDensiteEau);
	hr = D3DXCreateTextureFromFile(m_pD3DDevice, FichierDensiteEau,   &m_pTxDensiteEau);
	if(FAILED(hr))
		return false;
...
void CTerrain::RenduTerrain()
{
	...
	m_pD3DDevice->SetTexture(0, m_pTxHerbe);
	...
	if(m_pTxTerre != NULL && m_pTxDensiteTerre != NULL)
	{
		// On change l'index des coordonnées de textures
		...
		m_pD3DDevice->SetTexture(0, m_pTxDensiteTerre);
		m_pD3DDevice->SetTexture(1, m_pTxTerre);
	...
	if(m_pTxDensiteEau != NULL && m_pTxEau != NULL)
	{
		// Initialisation des stages de rendu (opération sur les textures)
		m_pD3DDevice->SetTextureStageState (0, D3DTSS_COLOROP,   D3DTOP_SELECTARG1);
		m_pD3DDevice->SetTextureStageState (0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
		...
		// Activation des textures
		m_pD3DDevice->SetTexture(0, m_pTxDensiteEau);
		m_pD3DDevice->SetTexture(1, m_pTxEau);
...
bool CTerrain::ChargerMapHauteur(char* FichierMapTerrain)
{
	...
	for(int j=0; j<m_NbPointY; j++)
	{
		...
		{
			TMapHauteur(i, j) = (float)ImageMapTerrain.GetPixel(i, j);
		}
	}
	return true;
}
