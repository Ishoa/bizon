...
CModelEau::CModelEau(CScene* pScene)
{
	...
	m_fLargeur			= 128.0f;	// largeur de l'eau
	m_fLongeur			= 256.0f;	// longeur de l'eau
	m_fPosition_Eau_X  = 127.0f;	// position en x de l'eau
	m_fPosition_Eaux_Y  = 12.0f;	// position en y de l'eau
	m_fPosition_Eaux_Z = 127.0f;
}
...
// Création de l'eau
bool CModelEau::CreationEau( float fPositionX, float fPositionY,float fPositionZ,float fLargeur, float fLongeur)
//bool CModelEau::CreationEau()

{


	m_fPosition_Eau_X = fPositionX;
	m_fPosition_Eaux_Y = fPositionY;
	m_fPosition_Eaux_Z = fPositionZ;

	m_fLargeur = fLargeur;
	m_fLongeur = fLongeur/NB_ZONE_EAU;
...
{
	static float OffsetZone = -float(m_fLongeur * NB_ZONE_EAU)/2.0f;
	...
	for(int i = 0 ; i < NB_ZONE_EAU ; i++)
	{
		...
		OffsetModel = OffsetZone + m_fPosition_Eaux_Z + m_fLongeur * i;
		//OffsetModel = float(m_fLongeur * i + (int(m_fPosition_Eaux_Z) / int(m_fLongeur)) * m_fLongeur) + OffsetZone;
