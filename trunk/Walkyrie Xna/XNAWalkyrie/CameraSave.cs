...
namespace XNAWalkyrie
{
    ...
    public class Camera : ICamera
    {
        ...
        public delegate void UpdateViewMatrixElapsedTime();

        public UpdateViewMatrixElapsedTime fonctionUpdateViewMatrixElapsedTime;
        ...
        public void Move(float dx, float dy, float dz)
        {
            ...
            switch (behavior)
            {
                ...
                case Behavior.ThirdPerson:
                        ...
                        if (fonctionUpdateViewMatrixElapsedTime != null)
                        {
                            fonctionUpdateViewMatrixElapsedTime();
                        }
                        else
                            UpdateViewMatrix();
                    break;
        ...
        private void ChangeBehavior(Behavior newBehavior)
        {
            ...
            switch (newBehavior)
            {
                ...
                case Behavior.Flight:
                    if (prevBehavior == Behavior.FreeOrbit || prevBehavior == Behavior.ThirdPerson)
                    {
                        //eye = savedEye;
                        //orientation = savedOrientation;
                        //accumPitchDegrees = savedAccumPitchDegrees;
                ...
                case Behavior.FreeOrbit:
                    {
                        if (prevBehavior == Behavior.ThirdPerson)
                            return;
                ...
                case Behavior.ThirdPerson:
                    {
                        if (prevBehavior == Behavior.FreeOrbit)
                            return;
        ...
        private void UpdateViewMatrix()
        {
            ...
            if (behavior == Behavior.FreeOrbit || behavior == Behavior.ThirdPerson)
            {
    ...
    public class CameraComponent : GameComponent, ICamera
    {
            ...
            : base(game)
        {
            ...
            camera.fonctionUpdateViewMatrixElapsedTime = UpdateViewMatrixElapsedTime;
        ...
        public void UpdateViewMatrixElapsedTime()
        {
            Quaternion cameraOrientation = camera.Orientation;
            Matrix CameraViewMatrix = camera.ViewMatrix;

            Matrix.CreateFromQuaternion(ref cameraOrientation, out CameraViewMatrix);
            camera.ViewMatrix = CameraViewMatrix;

            Vector3 tmpXAxis = new Vector3(camera.ViewMatrix.M11, camera.ViewMatrix.M21, camera.ViewMatrix.M31);
            camera.XAxis = tmpXAxis;

            Vector3 tmpYAxis = new Vector3(camera.ViewMatrix.M12, camera.ViewMatrix.M22, camera.ViewMatrix.M32);
            camera.YAxis = tmpYAxis;

            Vector3 tmpZAxis = new Vector3(camera.ViewMatrix.M13, camera.ViewMatrix.M23, camera.ViewMatrix.M33);
            camera.ZAxis = tmpZAxis;


            Vector3 idealPosition = camera.OrbitTarget + camera.ZAxis * camera.OrbitOffsetDistance;
            Vector3 displacement = camera.Position - idealPosition;
            Vector3 springAcceleration = (-springConstant * displacement) -
                                             (dampingConstant * velocity);

            currentVelocity.X += springAcceleration.X * camera.ElapsedTime;
            //camera.Position += currentVelocity * camera.ElapsedTime;


            camera.ZAxis = camera.Position - camera.OrbitTarget;
            camera.ZAxis.Normalize();

            tmpZAxis = camera.ZAxis;
            Vector3 tmpViewDir = camera.ViewDirection;
            Vector3.Negate(ref tmpZAxis, out tmpViewDir);
            camera.ViewDirection = tmpViewDir;

            Vector3 tmpTargetYaxis = camera.TargetYAxis;
            tmpXAxis = camera.XAxis;
            Vector3.Cross(ref tmpTargetYaxis, ref tmpZAxis, out tmpXAxis);
            tmpXAxis.Normalize();
            camera.XAxis = tmpXAxis;

            tmpYAxis = camera.YAxis;
            Vector3.Cross(ref tmpZAxis, ref tmpXAxis, out tmpYAxis);
            tmpYAxis.Normalize();
            camera.YAxis = tmpYAxis;

            Matrix viewMatrix;
            viewMatrix.M11 = camera.XAxis.X;
            viewMatrix.M21 = camera.XAxis.Y;
            viewMatrix.M31 = camera.XAxis.Z;
            Vector3 eye = camera.Position;
            Vector3.Dot(ref tmpXAxis, ref eye, out viewMatrix.M41);

            viewMatrix.M41 = -viewMatrix.M41;

            viewMatrix.M12 = tmpYAxis.X;
            viewMatrix.M22 = tmpYAxis.Y;
            viewMatrix.M32 = tmpYAxis.Z;

            Vector3.Dot(ref tmpYAxis, ref eye, out viewMatrix.M42);
            viewMatrix.M42 = -viewMatrix.M42;

            viewMatrix.M13 = tmpZAxis.X;
            viewMatrix.M23 = tmpZAxis.Y;
            viewMatrix.M33 = tmpZAxis.Z;
            Vector3.Dot(ref tmpZAxis, ref eye, out viewMatrix.M43);
            viewMatrix.M43 = -viewMatrix.M43;
        ...
        private void UpdateVelocity(ref Vector3 direction, float elapsedTimeSec)
        {
            ...
            currentVelocity += currentForce * elapsedTimeSec;
        }
