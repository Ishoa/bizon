...
const int   NB_ZONE_EAU     = 10;		// nombre de zone d'eau

const float COEF_TEXTURE_X	= 0.1f;		// répétition de la texture
const float COEF_TEXTURE_Y	= 0.1f;		// répétition de la texture
...
public:
	...
	float m_fPosition_Eaux_Z;

	CModelEau(CScene* pScene);
	~CModelEau();

	bool ChargerEau();
	bool CreationEau(float fPositionX, float fPositionY,float fPositionZ,float fLargeur, float fLongeur);
	//bool CreationEau();
	void Release();
	void RenduEau();
