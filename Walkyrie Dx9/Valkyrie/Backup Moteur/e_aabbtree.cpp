...
D3DXVECTOR3 E_AABBNode::FindPolygonsCenter()
{
    ...
    CPolygonContainer* p_Current = p_PolygonList->GetFirst();

    // On passe ensuite toute la liste en revue.
    while ( p_Current != NULL )
    {
        // On additionne le point central de chaque polygone.
        CPolygon Polygon = p_Current->GetPolygon()->GetClone();
...
E_Side E_AABBNode::TestPolygonWithBBoxPlane( CPolygon* Polygon,
                                             D3DXPLANE  BBPlane )
{
    float Result[3];
...
bool E_AABBNode::CreateChild( CPolygonList* PolygonList,
                              E_AABBTreeInfo Info,
                              E_AABBTree*    Root )
{
    ...
    CPolygonList* Left = new CPolygonList();
    ...
    CPolygonList* Right = new CPolygonList();
...
bool E_AABBNode::AddPolygons( CPolygonList* ResultList )
{
    ...
    CPolygonContainer* CurPolygon = p_PolygonList->GetFirst();
...
bool E_AABBNode::Resolve( CPolygonList* ResultList, CBoundingSphere Sphere )
{
    // On teste si la liste réponse est valide.
    if ( ResultList == NULL )
    {
    ...
    CBoundingBox Box = p_PolygonList->GetBoundingBox();
...
bool E_AABBTree::Initialize( CPolygonList* PolygonList )
{
    ...
    CPolygonList* p_New = new CPolygonList();
    ...
    CPolygonContainer* p_Current = PolygonList->GetFirst();
...
// Cette fonction retourne les polygones contenus dans la zone à tester.
bool E_AABBTree::Resolve( CPolygonList* Result, CBoundingSphere Sphere )
{
