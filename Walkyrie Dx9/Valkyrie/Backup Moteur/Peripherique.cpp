...
bool CPeripherique::Creation(DWORD Type)
{
	...
	if(Type && PERIPHERIQUE_JOYSTICK)
	{
		if(!m_pJoystick) 
			return false;
		if(!m_pJoystick->Creation(m_pDI))
		{
			delete m_pJoystick;
			m_pJoystick = NULL;
			Console<<" Aucun Joystick de disponible"<<endl;
		}
	}
	return true;
}
...
{
	if(m_pClavier)
		m_pClavier->Acquisition();
	if(m_pSouris)
		m_pSouris->Acquisition();
	if(m_pJoystick)
		m_pJoystick->Acquisition();
	return true;
}
...
bool CPeripherique::DeAcquisition()
{
	...
	{
		if(m_pClavier)
			m_pClavier->DeAcquisition();
		if(m_pSouris)
			m_pSouris->DeAcquisition();
		if(m_pJoystick)
			m_pJoystick->DeAcquisition();
	}
	return true;
}
...
{
	if(m_pClavier)
		m_pClavier->GetEtatClavier(bActiveAquisition);
	if(m_pSouris)
		m_pSouris->GetEtatSouris(bActiveAquisition);
	if(m_pJoystick)
		m_pJoystick->GetEtatJoystick(bActiveAquisition);
	return true;
}