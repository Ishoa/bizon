...
bool CMoteur3D::Creation(HWND hWnd, int Largeur, int Hauteur, bool bPleinEcran)
{
    ...
    m_d3dpp.AutoDepthStencilFormat = D3DFMT_D24S8;					// Format du Depth et Stencil
	...
	if (m_pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &m_d3dpp, &m_pD3DDevice) != D3D_OK)
	...
	D3DXCreateSprite(m_pD3DDevice, &m_pd3dspt);    // create the Direct3D Sprite object
...
bool CMoteur3D::RAZPeripherique3D()
{
    ...
    if(FAILED(hr = m_pD3DDevice->TestCooperativeLevel()))
    {
		...
		if(D3DERR_DEVICENOTRESET == hr)
		{
			...
			if(FAILED(m_pD3DDevice->Reset(&m_d3dpp)))
			{
				Console << "Impossible de réinitialiser le peripherique 3D" << endl;
				return false;
			}
			else
			{
